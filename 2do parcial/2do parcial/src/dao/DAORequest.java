...
public class DAORequest implements IDAORequest {
    ...
    public void insertar(Request r) throws DAOException {
        String sql = "INSERT INTO request (method, url, headers, body, response, timestamp, favorite_name) VALUES (?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
    ...
    @Override
    public List<Request> obtenerFavoritos() throws DAOException {
        List<Request> list = new ArrayList<>();
        String sql = "SELECT * FROM request WHERE favorite_name IS NOT NULL ORDER BY favorite_name";
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
             Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery(sql)) {
